...
func _ready() -> void:
	...
	for cell in cells:
		...
		if actors_node and actors_node.has_node("ActorPlayer"):
			...
			_camera_3d = _actor.get_node("Camera3D") as Camera3D
			break
...
func get_ray_intersection_position() -> Vector3:
	...
	var ray_origin:Vector3 = _camera_3d.project_ray_origin(mouse_pos)
	var ray_direction:Vector3 = _camera_3d.project_ray_normal(mouse_pos)
